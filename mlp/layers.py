...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        outputs = np.zeros((inputs.shape[0],self.num_output_channels, self.output_height ,self.output_width ))
        for i in range(N):
            for j in range(C_out):
                temp = np.zeros_like(outputs[i,j,:,:])
                for d in range (C_in):
                    temp+= scipy.signal.correlate2d(inputs[i,d,:,:],self.kernels[j,d,:,:],"valid") 
                outputs[i,j,:,:] = temp + self.biases[j]
        return outputs
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        for i in range(C_out):
            for j in range(C_in):
                temp = np.zeros_like(self.kernels[i,j,:,:])
                for d in range (N):
                    temp+= scipy.signal.correlate2d(inputs[d,j,:,:],grads_wrt_outputs[d,i,:,:],"valid") 
                grads_wrt_kernels[i,j,:,:] = temp
        grads_wtr_biases = np.sum(grads_wrt_outputs,axis = (0,2,3))
        return grads_wrt_kernels,grads_wtr_biases
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        grads_padded = np.pad(grads_wrt_outputs,npad,mode="constant")
        grads_wrt_inputs = np.zeros_like(inputs)
        
        ##grads_wrt_inputs
        for i in range(N):
            for j in range(C_in):
                temp = np.zeros_like(inputs[i,j,:,:])
                for d in range (C_out):
                    temp+= scipy.signal.correlate2d(grads_padded[i,d,:,:],self.kernels[d,j,::-1,::-1],"valid")  
                grads_wrt_inputs[i,j,:,:] = temp
...
class MaxPooling2DLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        batch,c_in,x_h,x_w = inputs.shape
        max_x = x_h // window_size
        max_y = x_w // window_size
        # Variables needed to re-arrange the matrix 
        #inputs has the max for the column and we want to compute the max for the rows 
        outputs = inputs.reshape(batch,c_in,x_h//window_size,window_size,x_w//window_size,window_size)
        self.cache = outputs
        print(outputs)
        #find the max on each line 
        outputs = outputs.max(3)
        outputs = outputs.max(4)
        return outputs 

#         inputs = inputs.reshape(batch,c_in,x_h,max_x,window_size).max(axis =4)
#         inputs= inputs.transpose(0,1,3,2)
#         _,_,x1,y1 = inputs.shape
#         inputs = inputs.reshape(batch,c_in,x1,y1//window_size,window_size)
#         inputs = inputs.max(axis=-1).reshape(batch,c_in,max_x,max_y).transpose(0,1,3,2)
#         self.cache = inputs
#         return inputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        outputs_reshaped = self.cache
        outputs_reshaped = outputs_reshaped[:,:,:,None,None]
        """
